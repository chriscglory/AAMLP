...
accuracy <- conf.matrix$overall["Accuracy"] %>% cat()
sensitivity <- conf.matrix$byClass["Sensitivity"] %>% cat()
specificity<- conf.matrix$byClass["Specificity"] %>% cat()
precision<- conf.matrix$byClass["Pos Pred Value"] %>% cat()
recall<- conf.matrix$byClass["Sensitivity"] %>% cat()
