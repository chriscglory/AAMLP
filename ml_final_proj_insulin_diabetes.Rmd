...
libs = c("tidyr","magrittr","purrr","dplyr","stringr","readr","data.table",
         "keras","ggplot2","imager","pROC","VIM","mice","partykit","caret","rpart","rpart.plot")
loadlibs(libs)
options(warn = -1)
```
...
diabetic_data=diabetic_data%>%select(-weight,-medical_specialty,-payer_code,-encounter_id,-patient_nbr,-change)
...
diabetic_data=diabetic_data %>% select(-diag)
...
lr.summary["insulinSteady",]
lr.summary["insulinSteady","Estimate"] %>% exp()-1

# pruned decision tree
dt.full = rpart(result ~ ., 
           data = traindata.imputed, control=rpart.control(xval=10, minsplit=10, cp=0.0001))
min.cv.idx <- which.min(lfv.full$cptable[,"xerror"])
# min CV error + 1se (this is the height of the horizontal bar)
min.cv.err.1se <- lfv.full$cptable[min.cv.idx,"xerror"] +
                    lfv.full$cptable[min.cv.idx,"xstd"]
# Which cp values produce models whose error is below min CV + 1se?
candidate.cp.vals <- lfv.full$cptable[which(lfv.full$cptable[,"xerror"] < min.cv.err.1se),"CP"]
# 1-SE rule value of cp
cp.1se <- max(candidate.cp.vals)

dt.pruned <- prune(dt.full, cp = cp.1se)

dt.pruned.party <- as.party(dt.pruned)
plot(dt.pruned.party, gp = gpar(fontsize = 5))
